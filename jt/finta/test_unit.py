...
for batch in pd.read_csv('AMD_dollar_all.csv', chunksize=batch_size):
    ohlc = batch
    ...
    batch['UO'] = TA.UO(ohlc)
    batch['CHAIKIN'] = TA.CHAIKIN(ohlc)
    batch['MFI'] = TA.MFI(ohlc)
    batch['VZO'] = TA.VZO(ohlc)
    batch['PZO'] = TA.PZO(ohlc)
    batch['EFI'] = TA.EFI(ohlc)
    batch['CFI'] = TA.CFI(ohlc)
    batch['CMO'] = TA.CMO(ohlc)
    batch['MSD'] = TA.MSD(ohlc)
