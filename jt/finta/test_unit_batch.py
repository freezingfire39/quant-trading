...
for batch in pd.read_csv('AMD_dollar_all.csv', chunksize=batch_size, index_col="date_time", parse_dates=True):
    ohlc = batch
    ohlc['volume'] = ohlc['cum_vol']
    count += 1
    #batch['AO'] = TA.AO(ohlc)
    #batch['EMA'] = TA.EMA(ohlc)
    #batch['DEMA'] = TA.DEMA(ohlc)
    #batch['TEMA'] = TA.TEMA(ohlc)
    #batch['TRIX'] = TA.TRIX(ohlc)
    #batch['TRIMA'] = TA.TRIMA(ohlc)
    #batch['VAMA'] = TA.VAMA(ohlc)
    #batch['ER'] = TA.ER(ohlc)
    #batch['ZLEMA'] = TA.ZLEMA(ohlc)
    #batch['HMA'] = TA.HMA(ohlc)
    #batch['EVWMA'] = TA.EVWMA(ohlc)
    #batch['TR'] = TA.TR(ohlc)
    #batch['ATR'] = TA.ATR(ohlc)
    #batch['SAR'] = TA.SAR(ohlc)
    #batch['UO'] = TA.UO(ohlc)
    batch['CHAIKIN_3_10'] = TA.CHAIKIN(ohlc, span_one=3, span_two=10)
    batch['CHAIKIN_8_20'] = TA.CHAIKIN(ohlc, span_one=8, span_two=20)
    batch['CHAIKIN_15_50'] = TA.CHAIKIN(ohlc, span_one=15, span_two=50)
    batch['CHAIKIN_16_64'] = TA.CHAIKIN(ohlc, span_one=16, span_two=64)
    batch['CHAIKIN_32_128'] = TA.CHAIKIN(ohlc, span_one=32, span_two=128)
    
    batch['MFI_10'] = TA.MFI(ohlc, period=10)
    batch['MFI_12'] = TA.MFI(ohlc, period=12)
    batch['MFI_14'] = TA.MFI(ohlc)
    batch['MFI_16'] = TA.MFI(ohlc, period=16)
    batch['MFI_18'] = TA.MFI(ohlc, period=18)
    #batch['VZO'] = TA.VZO(ohlc)
    #batch['PZO'] = TA.PZO(ohlc)
    batch['EFI_7'] = TA.EFI(ohlc,period=7)
    batch['EFI_10'] = TA.EFI(ohlc,period=10)
    batch['EFI_13'] = TA.EFI(ohlc)
    batch['EFI_16'] = TA.EFI(ohlc,period=16)
    batch['EFI_20'] = TA.EFI(ohlc,period=20)
    #batch['CFI'] = TA.CFI(ohlc)
    #batch['CMO'] = TA.CMO(ohlc)
    batch['MSD_15'] = TA.MSD(ohlc, period=15)
    batch['MSD_18'] = TA.MSD(ohlc, period=18)
    batch['MSD_21'] = TA.MSD(ohlc)
    batch['MSD_24'] = TA.MSD(ohlc, period=24)
    batch['MSD_27'] = TA.MSD(ohlc, period=27)
...
#df = pd.read_csv('AMD_dollar_all.csv', index_col="date_time", parse_dates=True)
#ohlc = pd.read_csv('AMD_dollar_all.csv', index_col="date_time", parse_dates=True)
#ohlc['volume'] = ohlc['cum_vol']



df.to_csv ('export_dataframe.csv', index = "date_time", header=True)
